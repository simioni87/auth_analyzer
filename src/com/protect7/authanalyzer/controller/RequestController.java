		...
		if (originalRequestResponse == null || originalRequestResponse.getRequest() == null) {
			...
			for (Session session : CurrentConfig.getCurrentConfig().getSessions()) {
				...
				if(!isFiltered) {
					...
					ArrayList<byte[]> modifiedRequests= RequestModifHelper.getModifiedRequests(originalRequestResponse.getRequest(), session, tokenPriority);

					for (byte[] modifiedRequest:modifiedRequests
						 ) {
						new PrintWriter(BurpExtender.callbacks.getStdout(),true).println("arrive: 76");
